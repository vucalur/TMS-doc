...
Zamiast tego chcemy dostarczyć produkt, który z domyślnymi ustawieniami adresuje potrzeby dużej rzeszy małych zespołów.
\begin{description}
	\item[Role użytkowników]
		Analogicznie do opisanej wyżej instalacji Redmine'a:
		\begin{itemize}
			\item	związana zarządzaniem systemem: administrator <<<<<< wogóle udostępniać taką rolę ? moim zdaniem nie,
					tylko admina (adminów) poszczególnego projektu
			\item	role pełnione przez użytkownika w danym projekcie, np:  developer, tester, manager
		\end{itemize}	związaną z zarządzaniem 
		Użytkownik i role są osobnymi bytami - użytkownik może mieć wiele ról, różne w różnych projektach.\\
		Użytkownik może być zarówno administratorem systemu jak i pełnić inne role w projektach.
	\item[Uprawnienienia użytkowników]
		Administratorzy mają nieograniczone prawa -- mogą np:zarządzać projektami nieutworzonymi przez siebie,
		czy też usuwać konta innych użytkowników, w tym administratorów.\je
		Każdy użytkownik może tworzyć projekty <<<<<< właśnie: będzie więcej bujania się jak robimy aplikację wieloskalową
		(każdy się może rejestrowac mailem, robić sobie projekty, itp.) także trzeba tutaj kwestie uprawnień dobrze przemyśleć
		żebyśmy potem nie klepali czegoś kiepskiego bo się szybko zrobi fuckup.\\
		Myślę że sensownie będzie zrobić coś jak bitbucket i przełożyć to na język tasków:\\
		repo = projekt
		na bb może być kilku adminów 1 repo 
	\item	\ldots
\end{description}
